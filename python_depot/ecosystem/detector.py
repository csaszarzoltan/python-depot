...
class PackageManagerDetector:
    ...
    def detect_from_pypi_json(cls, pypi_data: dict[str, Any]) -> dict[str, Any]:
        ...
        requires_dist = cls._extract_requires_dist_signals(
            pypi_data.get("info", {}).get("requires_dist")
        )
        url_signals = cls._extract_project_urls_signals(
            pypi_data.get("info", {}).get("project_urls")
        )
        classifier_signals = cls._extract_classifier_signals(
            pypi_data.get("info", {}).get("classifiers")
        )
        ...
        managers.add(cls.MANAGER_PIP)  # universal fallback
    ...
    @classmethod
    def _extract_requires_dist_signals(cls, requires_dist: list[str] | None) -> list[str]:
    ...
    @classmethod
    def _extract_project_urls_signals(cls, project_urls: dict[str, str] | None) -> list[str]:
    ...
    @classmethod
    def _extract_classifier_signals(cls, classifiers: list[str] | None) -> list[str]:
    ...
    def _parse_tool_sections(cls, toml_data: dict[str, Any]) -> dict[str, bool]:
        ...
        if not isinstance(tool, dict):
            return {"has_tool_uv": False, "has_tool_poetry": False, "has_tool_pip_tools": False}
        return {
