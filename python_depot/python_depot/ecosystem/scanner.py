...
Provides the `EcosystemScanner` class for per-package and batch scanning.
"""
...
class EcosystemScanner:
    ...
    Fetches PyPI JSON metadata, runs Tier 1 detection, and optionally
    inspects pyproject.toml content (Tier 2).

    Args:
        db: SQLAlchemy session for persisting scan results.
    """
    ...
    async def scan_package(
        ...
        Args:
            ...
            save: If True (default), persist the result to the database.

        Returns:
            Dict with scan results. Keys:
                - package: str
        ...
        if pypi_data is None:
            # Package not found or unreachable
            result = {
                "package": name,
        ...
        # Tier 1 detection from PyPI JSON
    ...
    async def scan_batch(
        ...
        Args:
            names: List of package names to scan.
            tier_1_only: If True, only perform Tier 1 (PyPI JSON) detection.
    ...
    async def _fetch_pypi_json(self, name: str) -> dict[str, Any] | None:
        ...
        Returns None if the package doesn't exist or the request fails.
        """
    ...
    def _infer_build_backend(self, pypi_data: dict[str, Any]) -> str | None:
        ...
        Uses classifiers and requires_dist hints.
        """
        # Try to detect from classifiers
    ...
    def _check_pyproject_availability(self, pypi_data: dict[str, Any]) -> bool:
        ...
        Heuristic: if the package has classifiers mentioning pyproject
        or modern build system, assume it has one.
        """
        classifiers = pypi_data.get("info", {}).get("classifiers") or []
        for c in classifiers:
            if "pyproject" in c.lower() or "project" in c.lower():
                return True
        has_backend = self._infer_build_backend(pypi_data) is not None
        return has_backend
