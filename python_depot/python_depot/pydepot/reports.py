...
class ReportGenerator:
    ...
    def generate_report(self, year: int, month: int) -> dict[str, Any]:
        ...
        if pkg_ids:
            for rating in self.db.query(Rating).filter(
                Rating.package_id.in_(pkg_ids)
            ):
        ...
        if pkg_ids:
            for scan in self.db.query(VulnerabilityScan).filter(
                VulnerabilityScan.package_id.in_(pkg_ids)
            ):
