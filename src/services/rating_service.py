...
class RatingService:
    ...
    def submit_rating(self, package_name: str, user_id: str, score: int) -> dict[str, Any]:
        """Submit or update a rating for a package.

        Args:
            package_name: Name of the package.
            user_id: User identifier (stub auth).
    ...
    def list_reviews(self, package_name: str, status: str | None = None) -> dict[str, Any]:
        """List reviews for a package, optionally filtered by status.
        ...
        return {
            "package": package_name,
            "reviews": [
                {"id": r.id, "title": r.title, "user_id": r.user_id, "status": r.moderation_status}
                for r in reviews
            ],
            "total": len(reviews),
        }
